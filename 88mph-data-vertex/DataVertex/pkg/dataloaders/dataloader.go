...
MPHById  MPHLoader
VestById  VestLoader
FractionalDepositById  FractionalDepositLoader
...
maxBatch: 100,
wait:     5 * time.Millisecond,
fetch: func(ids []string) ([]*models.Vest, []error) {
	placeholders := make([]string, len(ids))
	args := make([]interface{}, len(ids))
	for i := 0; i < len(ids); i++ {
		placeholders[i] = "?"
...
table, _ := db.GetTable("Vests")
valueById := map[string]*models.Vest{}

for _, key := range ids {
	result, err := table.Get(key, false)
	data := result.GetValue()

	var value models.Vest
	json.Unmarshal(data, &value)
	if err != nil {
		panic(err)
	}
...
values := make([]*models.Vest, len(ids))
for i, id := range ids {
	values[i] = valueById[id]
}
return values, nil
...
maxBatch: 100,
wait:     5 * time.Millisecond,
fetch: func(ids []string) ([]*models.FractionalDeposit, []error) {
	placeholders := make([]string, len(ids))
	args := make([]interface{}, len(ids))
	for i := 0; i < len(ids); i++ {
		placeholders[i] = "?"
...
table, _ := db.GetTable("FractionalDeposits")
valueById := map[string]*models.FractionalDeposit{}

for _, key := range ids {
	result, err := table.Get(key, false)
	data := result.GetValue()

	var value models.FractionalDeposit
	json.Unmarshal(data, &value)
	if err != nil {
		panic(err)
	}
...
values := make([]*models.FractionalDeposit, len(ids))
for i, id := range ids {
	values[i] = valueById[id]
}
return values, nil
